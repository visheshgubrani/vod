...
`clipmux_transcoder.agent` is the *execution* half of the shared engine: it owns
...
:mod:`clipmux_transcoder.pipeline`, shared byte-for-byte with the Modal runner.
...
Whisper. `clipmux-transcoder doctor` runs on a machine where none of them exist.
"""
...
def __getattr__(name: str):
    ...
    Deferred so `clipmux-transcoder version` does not pay for the HTTP client, the
    ...
    if name == "TranscoderApiClient":
        from clipmux_transcoder.agent.client import TranscoderApiClient

        return TranscoderApiClient
    if name == "JobRunner":
        from clipmux_transcoder.agent.runner import JobRunner
