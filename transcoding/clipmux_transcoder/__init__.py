"""
clipmux_transcoder — the shared ClipMux processing engine.
...
- the local worker (:mod:`clipmux_transcoder.agent`).
...
1. **Importing it costs nothing.** No Modal, no CUDA, no boto3, no Whisper, no
   ...
   needs them, so `import clipmux_transcoder` works on a bare machine and the
...
2. **The engine never authenticates to anything.** Transfers arrive as a
   parameter. This is what makes "the local worker holds no storage
   credentials" a property of the design rather than a promise in a document.
3. **Failures are typed.** :mod:`clipmux_transcoder.errors` carries the stable
...
4. **Hardware is proven, not assumed.** See
   :mod:`clipmux_transcoder.encoding.probe`.
"""
...
from clipmux_transcoder.options import (
    PLAYBACK_PUBLIC,
    PLAYBACK_SIGNED,
    PROVIDER_MODAL,
    PROVIDER_LOCAL,
...
from clipmux_transcoder.snapshot import SourceSnapshot, create_snapshot
...
def run_pipeline(*args, **kwargs):
    """
    Lazy re-export of :func:`clipmux_transcoder.pipeline.run_pipeline`.

    Deferred on purpose: `pipeline` pulls in the packaging and video modules,
    and keeping it out of the import graph means `import clipmux_transcoder`
    stays cheap for the agent's CLI when it only wants `--version` or `doctor`.
    """
    from clipmux_transcoder.pipeline import run_pipeline as _run
