...
export function printHelp(): void {
  ...
  const lines = [
    ...
    `                   ${color.muted('running the API on this machine via `pnpm dev`')}`,
    ...
    `  ${flag('--transcode')} modal|local   ${flag('--uploads')} on|off`,
    `  ${flag('--analytics')} on|off     ${flag('--db')} local|existing`,
    ...
    chalk.dim('    "db": { "kind": "local" },        // or { "kind":"existing", "url": "postgresql://…" }'),
    chalk.dim('    "analyticsEnabled": true,         // false = skip Analytics Engine writes'),
    chalk.dim('    "transcodeProvider": "modal",     // "modal" | "local"'),
    chalk.dim('    "uploadsEnabled": true,           // false = local files only (local worker)'),
    chalk.dim('    "queue": { "kind": "direct" },    // or { "kind": "qstash", "token": "…" }'),
    chalk.dim('    "rateLimit": { "kind": "memory" }, // or { "kind": "upstash", "restUrl": "…", "token": "…" }'),
    chalk.dim('                                      // or { "kind": "redis", "url": "redis://…" }'),
