...
function cachedError(xs2) {
  if (originCache.has(xs2))
    return originCache.get(xs2);
  const x2 = Error.stackTraceLimit;
  Error.stackTraceLimit = 4;
  originCache.set(xs2, new Error());
...
var Builder = class extends NotTagged {
  ...
  build(before, parameters, types2, options) {
    const keyword = builders.map(([x2, fn]) => ({ fn, i: before.search(x2) })).sort((a2, b2) => a2.i - b2.i).pop();
...
function escapeIdentifiers(xs2, { transform: { column } }) {
  return xs2.map((x2) => escapeIdentifier(column.to ? column.to(x2) : x2)).join(",");
}
...
var arraySerializer = function arraySerializer2(xs2, serializer, options, typarray) {
  if (Array.isArray(xs2) === false)
    return xs2;
  if (!xs2.length)
    return "{}";
  const first = xs2[0];
  ...
  if (Array.isArray(first) && !first.type)
    return "{" + xs2.map((x2) => arraySerializer2(x2, serializer, options, typarray)).join(delimiter) + "}";
  return "{" + xs2.map((x2) => {
    if (x2 === void 0) {
  ...
  for (; s.i < x2.length; s.i++) {
    ...
    if (s.quoted) {
      if (s.char === "\\") {
        ...
        xs2.push(parser ? parser(s.str) : s.str);
        s.str = "";
        s.quoted = x2[s.i + 1] === '"';
        s.last = s.i + 2;
      } else {
      ...
      s.last = ++s.i;
      xs2.push(arrayParserLoop(s, x2, parser, typarray));
    } else if (s.char === "}") {
      s.quoted = false;
      s.last < s.i && xs2.push(parser ? parser(x2.slice(s.last, s.i)) : x2.slice(s.last, s.i));
      s.last = s.i + 1;
      break;
    } else if (s.char === delimiter && s.p !== "}" && s.p !== '"') {
      xs2.push(parser ? parser(x2.slice(s.last, s.i)) : x2.slice(s.last, s.i));
      s.last = s.i + 1;
    }
    s.p = s.char;
  }
  s.last < s.i && xs2.push(parser ? parser(x2.slice(s.last, s.i + 1)) : x2.slice(s.last, s.i + 1));
  return xs2;
}
var toCamel = (x2) => {
  let str = x2[0];
...
function Queue(initial = []) {
  let xs2 = initial.slice();
  let index2 = 0;
  return {
    get length() {
      return xs2.length - index2;
    },
    remove: (x2) => {
      const index3 = xs2.indexOf(x2);
      return index3 === -1 ? null : (xs2.splice(index3, 1), x2);
    },
    push: (x2) => (xs2.push(x2), x2),
    shift: () => {
      const out = xs2[index2++];
...
function Connection(options, queues = {}, { onopen = noop, onend = noop, onclose = noop } = {}) {
  ...
  function addArrayType(oid, typarray) {
    ...
    options.parsers[typarray] = (xs2) => arrayParser(xs2, parser, typarray);
    options.parsers[typarray].array = true;
    options.serializers[typarray] = (xs2) => arraySerializer(xs2, options.serializers[oid], options, typarray);
  }
  function tryNext(x2, xs2) {
    return x2 === "read-write" && xs2.default_transaction_read_only === "on" || x2 === "read-only" && xs2.default_transaction_read_only === "off" || x2 === "primary" && xs2.in_hot_standby === "on" || x2 === "standby" && xs2.in_hot_standby === "off" || x2 === "prefer-standby" && xs2.in_hot_standby === "off" && options.host[retries];
  }
  function fetchState() {
...
function md5(x2) {
  return import_crypto.default.createHash("md5").update(x2).digest("hex");
}
function hmac(key, x2) {
  return import_crypto.default.createHmac("sha256", key).update(x2).digest();
}
function sha256(x2) {
  return import_crypto.default.createHash("sha256").update(x2).digest();
}
function xor(a2, b2) {
...
function Subscribe(postgres2, options) {
  ...
  async function init(sql3, slot2, publications) {
    ...
    return { stream: stream2, state: xs2.state };
    function error(e) {
    ...
    function handle(a2, b2) {
      ...
      call("*", a2, b2);
      call("*:" + path2, a2, b2);
      b2.relation.keys.length && call("*:" + path2 + "=" + b2.relation.keys.map((x3) => a2[x3.name]), a2, b2);
      call(b2.command, a2, b2);
      call(b2.command + ":" + path2, a2, b2);
      b2.relation.keys.length && call(b2.command + ":" + path2 + "=" + b2.relation.keys.map((x3) => a2[x3.name]), a2, b2);
    }
    function pong() {
  ...
  function call(x2, a2, b2) {
    subscribers.has(x2) && subscribers.get(x2).forEach(({ fn }) => fn(a2, b2, x2));
  }
}
function Time(x2) {
...
function parse(x2, state, parsers2, handle, transform) {
  const char2 = (acc, [k, v2]) => (acc[k.charCodeAt(0)] = v2, acc);
  Object.entries({
    R: (x3) => {
      let i = 1;
    ...
    D: (x3) => {
      ...
      const key = x3[i] === 75;
      handle(
        key || x3[i] === 79 ? tuples(x3, relation.columns, i += 3, transform).row : null,
        {
          command: "delete",
          relation,
          key
    ...
    U: (x3) => {
      ...
      i += 4;
      const key = x3[i] === 75;
      const xs2 = key || x3[i] === 79 ? tuples(x3, relation.columns, i += 3, transform) : null;
      xs2 && (i = xs2.i);
...
function parseEvent(x2) {
  const xs2 = x2.match(/^(\*|insert|update|delete)?:?([^.]+?\.?[^=]+)?=?(.+)?/i) || [];
  if (!xs2)
...
function parseOptions(a2, b2) {
  if (a2 && a2.shared)
    return a2;
  const env = process.env, o = (!a2 || typeof a2 === "string" ? b2 : a2) || {}, { url, multihost } = parseUrl(a2), query = [...url.searchParams].reduce((a3, [b3, c]) => (a3[b3] = c, a3), {}), host = o.hostname || o.host || multihost || url.hostname || env.PGHOST || "localhost", port = o.port || url.port || env.PGPORT || 5432, user2 = o.user || o.username || url.username || env.PGUSERNAME || env.PGUSER || osUsername();
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/entity.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/logger.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/query-promise.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/column.js
var Column = class {
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/column-builder.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/table.utils.js
var TableName = /* @__PURE__ */ Symbol.for("drizzle:Name");

// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/foreign-keys.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/tracing-utils.js
function iife(fn, ...args) {
  return fn(...args);
}

// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/unique-constraint.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/utils/array.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/common.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/enum.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/subquery.js
var Subquery = class {
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/version.js
var version = "0.45.2";

// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/tracing.js
var otel;
var rawTracer;
var tracer = {
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/view-common.js
var ViewBaseConfig = /* @__PURE__ */ Symbol.for("drizzle:ViewBaseConfig");

// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/table.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/sql/sql.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/alias.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/selection-proxy.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/utils.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/int.common.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/bigint.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/bigserial.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/boolean.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/char.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/cidr.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/custom.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/date.common.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/date.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/double-precision.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/inet.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/integer.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/interval.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/json.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/jsonb.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/line.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/macaddr.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/macaddr8.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/numeric.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/point.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/postgis_extension/utils.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/postgis_extension/geometry.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/real.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/serial.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/smallint.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/smallserial.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/text.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/time.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/timestamp.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/uuid.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/varchar.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/vector_extension/bit.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/vector_extension/halfvec.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/vector_extension/sparsevec.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/vector_extension/vector.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/columns/all.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/table.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/indexes.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/primary-keys.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/casing.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/errors.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/sql/expressions/conditions.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/sql/expressions/select.js
function asc(column) {
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/relations.js
var Relation = class {
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/view-base.js
var PgViewBase = class extends View {
  static [entityKind] = "PgViewBase";
};

// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/dialect.js
var PgDialect = class {
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/query-builders/query-builder.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/query-builders/select.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/query-builders/query-builder.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/utils.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/query-builders/delete.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/query-builders/insert.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/query-builders/refresh-materialized-view.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/query-builders/update.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/query-builders/count.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/query-builders/query.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/query-builders/raw.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/db.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/cache/core/cache.js
var Cache = class {
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/pg-core/session.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/postgres-js/session.js
...
var PostgresJsSession = class _PostgresJsSession extends PgSession {
  ...
  transaction(transaction, config) {
    return this.client.begin(async (client) => {
      const session2 = new _PostgresJsSession(
        client,
        this.dialect,
        this.schema,
        this.options
...
var PostgresJsTransaction = class _PostgresJsTransaction extends PgTransaction {
  constructor(dialect, session2, schema, nestedIndex = 0) {
    super(dialect, session2, schema, nestedIndex);
    this.session = session2;
  }
  ...
  transaction(transaction) {
    return this.session.client.savepoint((client) => {
      ...
      const tx = new _PostgresJsTransaction(this.dialect, session2, this.schema);
      return transaction(tx);
    });
  }
};

// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/postgres-js/driver.js
...
function construct(client, config = {}) {
  ...
  const session2 = new PostgresJsSession(client, dialect, schema, { logger, cache: config.cache });
  const db2 = new PostgresJsDatabase(dialect, session2, schema);
  db2.$client = client;
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/migrator.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/postgres-js/migrator.js
...
var Ln = I((Ft) => {
  ...
  Ft.write = function(r, e, t, n, i, s) {
    var o, u, c, h = s * 8 - i - 1, l = (1 << h) - 1, d = l >> 1, b2 = i === 23 ? Math.pow(2, -24) - Math.pow(2, -77) : 0, C = n ? 0 : s - 1, B = n ? 1 : -1, Q = e < 0 || e === 0 && 1 / e < 0 ? 1 : 0;
...
var ji = I((el, Qi) => {
  ...
  function ru(r, e, t) {
    ...
    var i = y.from(n.salt, "base64"), s = uu(
      ...
      i,
      n.iteration
    ), o = De(s, "Client Key"), u = au(o), c = "n=*,r=" + r.clientNonce, h = "r=" + n.nonce + ",s=" + n.salt + ",i=" + n.iteration, l = "c=biws,r=" + n.nonce, d = c + "," + h + "," + l, b2 = De(u, d), C = qi(
      o,
      b2
...
var bt = I((Il, us) => {
  "use strict";
  p();
  var Ru = (rs(), O(ts)), as = et(), os2 = ss().parse, V = a(
    function(r, e, t) {
    ...
    constructor(e) {
      e = typeof e == "string" ? os2(e) : e || {}, e.connectionString && (e = Object.assign({}, e, os2(e.connectionString))), this.user = V("user", e), this.database = V("database", e), this.database === void 0 && (this.database = this.user), this.port = parseInt(
        V("port", e),
        10
...
var ys = I((Ml, ds) => {
  ...
  var { EventEmitter: Du } = ge(), fs3 = ls(), ps = tt(), _r = class _r extends Du {
    constructor(e, t, n) {
      super(), e = ps.normalizeQueryConfig(e, t, n), this.text = e.text, this.values = e.values, this.rows = e.rows, this.types = e.types, this.name = e.name, this.binary = e.binary, this.portal = e.portal || "", this.callback = e.callback, this._rowMode = e.rowMode, m.domain && e.callback && (this.callback = m.domain.bind(e.callback)), this._result = new fs3(this._rowMode, this.types), this._results = this._result, this.isPreparedStatement = false, this._canceledDueToError = false, this._promise = null;
    }
    requiresPreparation() {
...
var An = class An2 extends Bt.Client {
  ...
  async _handleAuthSASLContinue(t) {
    ...
    let l = parseInt(h, 10), d = y.from(c, "base64"), b2 = new TextEncoder(), C = b2.encode(i), B = await w.subtle.importKey("raw", C, { name: "HMAC", hash: { name: "SHA-256" } }, false, ["sign"]), Q = new Uint8Array(await w.subtle.sign("HMAC", B, y.concat([d, y.from(
      [0, 0, 0, 1]
    )]))), X = Q;
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/neon-http/session.js
var rawQueryConfig = {
  arrayMode: false,
  fullResults: true
};
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/neon-http/driver.js
...
// ../node_modules/.pnpm/drizzle-orm@0.45.2_@cloudflare+workers-types@4.20260702.1_@neondatabase+serverless@0.10_2a83aa23e66c0be837b98ef1041d7ea4/node_modules/drizzle-orm/neon-http/migrator.js
...
__export(schema_exports, {
  ...
  playbackPolicyEnum: () => playbackPolicyEnum,
  session: () => session,
...
var statusEnum = pgEnum("video_status", [
  "pending",
  "uploading",
  "processing",
  "ready",
...
var playbackPolicyEnum = pgEnum("playback_policy", [
  "public",
  "signed"
]);
var video = pgTable("video", {
  ...
  // URL to the generated subtitle file (VTT/SRT)
  // AI Chapters generation
  generateChapters: boolean("generate_chapters").default(false),
  chaptersStatus: text("chapters_status"),
  // 'pending' | 'processing' | 'completed' | 'failed'
  chapters: jsonb("chapters").$type(),
  // Storage tracking for billing
  transcodedSize: bigint("transcoded_size", { mode: "number" }),
  // Total bytes of transcoded files (HLS + poster + subtitles)
  // Processing metrics
  transcodedTime: integer("transcoded_time"),
  // Transcoding duration in seconds
  // Job lifecycle tracking (heartbeats, sweeps, retries)
  processingStartedAt: timestamp("processing_started_at"),
  // when the current job attempt started
  jobAttempts: integer("job_attempts").default(0),
  // successful dispatch attempts
  lastHeartbeatAt: timestamp("last_heartbeat_at"),
  // last heartbeat from the transcoder
  failureCode: text("failure_code"),
  // typed machine-readable failure reason
...
var apiKey = pgTable(
  "api_key",
  {
    ...
    // SHA-256 digest of the secret key
    keyLast4: text("key_last4").notNull(),
    // Last 4 chars for masked previews
    name: text("name").notNull(),
    // "Production Key"
...
var webhookEndpoint = pgTable("webhook_endpoint", {
  id: text("id").primaryKey(),
  // "whep_..."
  organizationId: text("organization_id").notNull().references(() => organization.id, { onDelete: "cascade" }),
  ...
  // ['video.ready', 'subtitle.generated', ...]
  enabled: boolean("enabled").default(true),
  description: text("description"),
  lastTriggeredAt: timestamp("last_triggered_at"),
  createdAt: timestamp("created_at").defaultNow()
});
var uploadToken = pgTable("upload_token", {
  id: text("id").primaryKey(),
  // "ut_xxxxx"
  token: text("token").notNull().unique(),
  ...
  // How many uploads allowed with this token
  usedFiles: integer("used_files").default(0),
  // How many uploads have been used
  maxSizeBytes: bigint("max_size_bytes", { mode: "number" }),
  // Optional size limit per file
  expiresAt: timestamp("expires_at").notNull(),
  createdAt: timestamp("created_at").defaultNow()
});
...
var videoRelations = relations(video, ({ one }) => ({
  organization: one(organization, {
...
var cachedDb = null;
var cachedKey = null;
function getDb(databaseUrl, env) {
  const url = databaseUrl || env?.DATABASE_URL || (typeof process !== "undefined" ? process.env?.DATABASE_URL : void 0);
  if (!url) {
    throw new Error("DATABASE_URL is not configured");
  }
  const driver = dbDriverFromEnv(
    env?.DB_DRIVER ?? (typeof process !== "undefined" ? process.env?.DB_DRIVER : void 0)
  );
  ...
  if (driver === "postgres-js") {
    const client = src_default(url, postgresOptions(url));
    cachedDb = drizzle(client, { schema: schema_exports });
  } else {
    const sql2 = Xs(url);
    cachedDb = drizzle2(sql2, { schema: schema_exports });
  }
  cachedKey = key;
  return cachedDb;
}
...
// src/node/migrate.ts
var MIGRATION_LOCK_KEY = 872342011;
async function main() {
  ...
  const migrationsFolder = import_node_path.default.join(process.cwd(), "drizzle");
  if (driver === "postgres-js") {
    const client = src_default(url, { max: 1 });
    try {
      await client`select pg_advisory_lock(${MIGRATION_LOCK_KEY})`;
      const db3 = drizzle(client, { schema: schema_exports });
      await migrate(db3, { migrationsFolder });
      console.log("[migrate] schema up to date (postgres-js)");
    } finally {
      await client`select pg_advisory_unlock(${MIGRATION_LOCK_KEY})`.catch(() => void 0);
      await client.end().catch(() => void 0);
    }
    return;
  }
  const db2 = drizzle2(Xs(url), { schema: schema_exports });
  await migrate2(db2, { migrationsFolder });
