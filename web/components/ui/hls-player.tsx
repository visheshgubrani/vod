  ...
  useEffect(() => {
    ...
    hls.on(Hls.Events.MANIFEST_PARSED, (event, data) => {
      ...
      data.levels.forEach((level, i) => {
        addLog(
          "info",
          `  Level ${i}: ${level.width}x${level.height} @ ${Math.round(
            level.bitrate / 1000
          )}kbps`
        );
      });
      ...
      if (autoPlay) {
        video
          .play()
          .catch((e) => addLog("warn", `Autoplay failed: ${e.message}`));
      }
    });
    ...
    hls.on(Hls.Events.LEVEL_SWITCHED, (event, data) => {
      ...
      addLog(
        "event",
        `Level switched to ${data.level}: ${level?.width}x${level?.height}`
      );
    });
    ...
    hls.on(Hls.Events.FRAG_LOADED, (event, data) => {
      addLog(
        "info",
        `Fragment loaded: ${data.frag.sn}, size: ${Math.round(
          data.frag.stats.total / 1024
        )}KB`
      );
    });
    ...
    hls.on(Hls.Events.ERROR, (event, data) => {
      ...
      addLog(
        "error",
        `  Fatal: ${data.fatal}, Reason: ${data.reason || "unknown"}`
      );
        ...
        setHlsStats({
          buffered:
            video.buffered.length > 0
              ? `${video.buffered.start(0).toFixed(1)}s - ${video.buffered
    ...
    const handleError = (e: Event) => {
      ...
      addLog(
        "error",
        `Video element error: ${mediaError?.code} - ${mediaError?.message}`
      );
    };
    const handleSeeking = () => {
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
        ...
        <input
          ...
          value={currentTime}
          onChange={handleSeek}
          className="w-full h-1 mb-3 bg-white/30 rounded-sm appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:rounded-full"
        />
        ...
        <div className="absolute top-0 left-0 right-0 max-h-[60%] overflow-y-auto bg-black/90 p-3 text-xs font-mono">
          ...
          {hlsStats && (
            <div className="mb-2 p-2 bg-blue-500/20 rounded text-blue-300">
              ...
              <div>
                Current Level: {hlsStats.currentLevel} | Next:{" "}
                {hlsStats.nextLevel} | Load: {hlsStats.loadLevel}
              </div>
            </div>
          )}
