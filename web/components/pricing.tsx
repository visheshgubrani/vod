      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-base md:text-lg text-muted-foreground max-w-2xl mx-auto">
            Simple, predictable pricing with no surprises. Pay only for what you
            use.
          </p>
        </div>

        {/* Cost Calculator */}
        <div className="border-card border bg-card/60 rounded-3xl p-8 mb-16 max-w-4xl mx-auto">
          <h3 className="text-lg md:text-xl font-medium mb-8 text-center">
            Cost Comparison Calculator
          </h3>

          <div className="mb-8">
            <label className="block text-sm text-muted-foreground mb-3">
              Monthly encoding minutes:{" "}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Our Cost */}
            <div className="rounded-2xl bg-gradient-to-br from-primary/10 to-accent/20 p-6 text-center border border-primary/30">
            ...
            {/* Savings */}
            <div className="rounded-2xl bg-lime-900/20 p-6 text-center border border-lime-500/20">
              <div className="text-sm text-muted-foreground mb-2">You Save</div>
              <div className="text-3xl font-bold text-lime-600">
                ${savings.toFixed(2)}
              </div>
              <div className="text-xs text-lime-600/80 mt-1">
        ...
        {/* Pricing Tiers */}
        <div className="grid grid-cols-1 lg:max-w-full max-w-md mx-auto lg:grid-cols-3 gap-8">
          {tiers.map((tier, index) => (
            <div
              key={index}
              className={`relative bg-mauve-600/20 rounded-3xl p-8 ${
                tier.highlighted
                  ? "ring-2 ring-mauve-500/50 scale-105 shadow-xl"
                  : ""
              }`}
            >
              {tier.highlighted && (
                <div className="items-center gap-2 bg-mauve-700 px-4 inline-flex rounded-full border-mauve-300/25 border bg-card  absolute -top-4 left-1/2 -translate-x-1/2">
              ...
              <ul className="space-y-3 mb-8">
                {tier.features.map((feature, i) => (
                  <li key={i} className="flex items-center gap-3">
                    {feature.included ? (
                      <Check className="w-5 h-5 text-lime-600/90 flex-shrink-0" />
                    ) : (
              ...
              <Button
                className={
                  tier.highlighted
                    ? "w-full h-auto py-2.5 rounded-full bg-mauve-600 text-white hover:bg-mauve-500/90"
                    : "w-full h-auto py-2.5 bg-transparent bg-mauve-800/80 hover:bg-mauve-700/70 font-semibold"
                }
              >
                {tier.cta}
              </Button>
